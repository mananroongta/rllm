...
from rllm.agents.system_prompts import (
    FIN_QA_CORRECTNESS_PROMPT,
    FIN_QA_MULTI_TABLE_CORRECTNESS_PROMPT
)
...
JUDGE_MODEL = "gpt-4o"
MULTI_TABLE_JUDGE_MODEL = "gpt-4o-mini"
# Note: Results dir logic can be adapted or removed if not used for logging
...
try:
    # Only initialize if keys are present to avoid startup errors if just importing
    if PORTKEY_API_KEY and OPENAI_API_KEY:
        ...
        # Fallback or None - will cause _call_judge to return 0/False gracefully
        JUDGE_CLIENT = None
...
def _call_judge(
    ...
    if JUDGE_CLIENT is None:
        # If no client, we cannot judge. Return failure/0.
    ...
    if is_multi_table:
        ...
        # Portkey/OpenAI structured outputs
    ...
    try:
        ...
        print(f"Error calling judge: {exc}")
        return (False if not is_multi_table else 0.0), {}
...
def fin_qa_reward_function(task_info: dict, action: str) -> RewardOutput:
    ...
    if is_multi_table:
        ...
        system_prompt = FIN_QA_MULTI_TABLE_CORRECTNESS_PROMPT
    else:
        ...
        system_prompt = FIN_QA_CORRECTNESS_PROMPT
