...
from rllm.tools.fin_qa_constants import TABLES_ROOT, TABLES_CLEANED_ALL_COMPANIES_FILE_NAME
...
def _compute_table_info(table: str, metadata: dict, df: pd.DataFrame) -> str:
    """Pre-compute get_table_info response."""
...
def _ensure_loaded():
    """Ensure data is loaded into memory/sqlite. Called lazily."""
    ...
    with _DB_LOCK:
        if _PRELOADED:  # Double-check locking
            return
        ...
        if not os.path.exists(TABLES_ROOT):
            warnings.warn(f"FinQA Tables Root not found at: {TABLES_ROOT}. Tools requiring data will fail.")
            _PRELOADED = True
            return
